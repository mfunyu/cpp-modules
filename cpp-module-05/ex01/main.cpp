...
/*   Updated: 2021/11/19 14:47:50 by mfunyu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::string createFormString(
	std::string name, int gradeToSign, int gradeToExec, bool isSigned = false)
{
...
void printFormTestInfo(std::string name, int gradeToSign, int gradeToExec,
	bool isSigned = false, std::string append = "")
{
	std::cout << SET_COLOR "// "
			  << createFormString(name, gradeToSign, gradeToExec, isSigned)
			  << append << RESET_COLOR << std::endl;
}

void printBureaucratTestInfo(
	std::string name, int grade, std::string append = "")
{
...
{
	std::string name = "RandomForm";
	printFormTestInfo(name, gradeToSign, gradeToExec, isSigned,
...
void testAssignationOperator(int gradeToSign, int gradeToExec, bool isSigned)
{
	std::string name = "RandomForm";
	printFormTestInfo(name, gradeToSign, gradeToExec, isSigned,
		" [ only sign status will be the same ]");
...
void testFormBeSigned(int bureaucreatGrade, int gradeToSign, int gradeToExec)
{
	...
	printFormTestInfo(f_name, gradeToSign, gradeToExec, false,
		".beSigned(" + createBureaucratString(b_name, bureaucreatGrade) + ')');
...
void testBureaucratSignForm(
	int bureaucreatGrade, int gradeToSign, int gradeToExec)
{
	std::string b_name = "Juliet";
	std::string f_name = "UnimportantForm";
	printBureaucratTestInfo(b_name, bureaucreatGrade,
		".SignForm(" + createFormString(f_name, gradeToSign, gradeToExec)
			+ ')');
